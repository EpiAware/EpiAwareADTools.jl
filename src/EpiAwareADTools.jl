...
[`logsumexp_stream`](@ref) is the shared streaming log-sum-exp accumulator for
an infinite series over an unbounded discrete support, stopping only after a
run of consecutive terms has left the total unchanged rather than at the
first negligible term.
