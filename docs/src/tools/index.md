...
| [`cdf_ad_safe`](@ref), [`logcdf_ad_safe`](@ref), [`ccdf_ad_safe`](@ref), [`logccdf_ad_safe`](@ref), [`pdf_ad_safe`](@ref) | Extension-point wrappers whose `Gamma`, `Beta` and [Student-t](@ref student-t-hooks) methods stay differentiable in shape/scale (and, for the t, degrees of freedom) | `Distributions.jl` Gamma/Beta/Student-t CDF/log-CDF differentiable in their parameters | `cdf`/`logcdf` on `Gamma`, `Beta` and `TDist` differentiable upstream on the supported backends |
