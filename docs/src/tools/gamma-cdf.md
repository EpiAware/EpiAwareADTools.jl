...
The shape partial divides `_gamma_cdf_value_and_partials`'s series by `gamma_inc`'s accurate survival while `Q ≥ 1e-8`, hands over to `_dlogQ_da_tail_series`'s corrected asymptotic expansion beyond, and carries the same per-backend rules as `_gamma_cdf`.
