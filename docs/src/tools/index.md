...
| [`cdf_ad_safe`](@ref), [`logcdf_ad_safe`](@ref), [`ccdf_ad_safe`](@ref), [`logccdf_ad_safe`](@ref), [`pdf_ad_safe`](@ref) | Extension-point wrappers whose `Gamma`, `Beta` and `TDist` methods stay differentiable in shape/scale (and, for a t, in its degrees of freedom and its location and scale) | `Distributions.jl` Gamma/Beta/Student-t CDF/log-CDF differentiable in their parameters | `cdf(::Gamma)`/`cdf(::Beta)` and `logcdf(::Gamma)`/`logcdf(::Beta)` differentiable upstream on the supported backends, which also settles the Student-t |
