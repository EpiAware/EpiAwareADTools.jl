...
`_gamma_logccdf_value_and_partials` reuses `_gamma_cdf_value_and_partials`'s `(dk, dθ, dx)` divided by this accurate survival rather than by a literal `1 - P`, and carries the same per-backend rules as `_gamma_cdf`.
