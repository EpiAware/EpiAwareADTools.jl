...
`EpiAwareADTools` adopts shared configuration and test infrastructure from
...
Two pieces keep `EpiAwareADTools` aligned with the kit.
...
update("path/to/EpiAwareADTools.jl")
```
